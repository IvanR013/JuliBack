...
public class CloudinaryService
{
    ...
    public async Task<string> UploadImageAsync(IFormFile file)
    {
        ...
        try
        {
            ...
            if (uploadResult == null) throw new InvalidOperationException("El resultado de la carga es nulo.");

            if (uploadResult.StatusCode != System.Net.HttpStatusCode.OK) throw new InvalidOperationException($"Error en la carga de la imagen: {uploadResult.Error?.Message}");

            if (uploadResult.SecureUrl == null) throw new InvalidOperationException("La URL segura no fue generada correctamente.");
