...
public partial class ImageRep : IimageRepository
{
    ...
    public ImageRep(AppDbContext context) => this._Context = context;
...
public partial class ImageRep
{
    ...
    public async Task<List<Images>> GetByIdAsync(List<int> ids) => await _Context.Images.Where(img => ids.Contains(img.Id)).ToListAsync();

    //Lógica de borrado.
    public async Task DeleteAsync(List<int> id)
    {
        List<Images>? img = await GetByIdAsync(id);

        _Context.Images.RemoveRange(img);

        await _Context.SaveChangesAsync();
    }
    public async Task<List<Images>> GetAllImagesAsync() => await _Context.Images.ToListAsync();
}
...
>>>>>>> a8276bf834d8e2496eb5226f72e6274af122c16c
